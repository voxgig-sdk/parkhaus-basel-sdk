...
class ParkhausBaselTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ParkhausBaselTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ParkhausBaselTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ParkhausBaselTestRunner.getenv("PARKHAUS_BASEL_TEST_LIVE")
        override = ParkhausBaselTestRunner.getenv("PARKHAUS_BASEL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ParkhausBaselTestRunner.getenv(key)
        ...
        explain = ParkhausBaselTestRunner.getenv("PARKHAUS_BASEL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PARKHAUS_BASEL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ParkhausBaselTestRunner._test_control is not None:
            return ParkhausBaselTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ParkhausBaselTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ParkhausBaselTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ParkhausBaselTestRunner.load_test_control()
...
def load_env_local():
    ParkhausBaselTestRunner.load_env_local()


def env_override(m):
    return ParkhausBaselTestRunner.env_override(m)


def entity_data(v):
    return ParkhausBaselTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ParkhausBaselTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ParkhausBaselTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ParkhausBaselTestRunner.load_test_control()


def live_client_options():
    return ParkhausBaselTestRunner.live_client_options()


def live_delay_ms():
    return ParkhausBaselTestRunner.live_delay_ms()
