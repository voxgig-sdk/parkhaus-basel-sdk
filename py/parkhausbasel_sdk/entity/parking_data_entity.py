...
from parkhausbasel_sdk.parkhausbasel_types import (
    ParkingData,
...
class ParkingDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ParkingDataEntity(self._client, opts)
    ...
    def match_get(self) -> ParkingData:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ParkingData:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ParkingData().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ParkingData]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ParkingData().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
