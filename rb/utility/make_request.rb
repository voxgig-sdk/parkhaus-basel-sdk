...
module ParkhausBaselUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ParkhausBaselResponse.new({})
    ...
    if fetch_err
      ...
      response = ParkhausBaselResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ParkhausBaselResponse.new(fetched)
    else
