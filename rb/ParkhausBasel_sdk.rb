...
require_relative 'ParkhausBasel_types'
...
class ParkhausBaselSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ParkhausBaselHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ParkhausBaselHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ParkhausBaselFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ParkhausBaselUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ParkhausBaselHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ParkhausBaselHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ParkhausBaselHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ParkhausBaselHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ParkhausBaselError.new(
        "graphql_error", "ParkhausBaselSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ParkhausBaselSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
