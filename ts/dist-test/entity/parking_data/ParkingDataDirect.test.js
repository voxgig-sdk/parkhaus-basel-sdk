...
(0, node_test_1.describe)('ParkingDataDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.ParkhausBaselSDK({
    ...
    (0, node_test_1.test)('direct-load-parking_data', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-parking_data', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-parking_data', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-parking_data', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.PARKHAUS_BASEL_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ParkhausBaselSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['PARKHAUS_BASEL_TEST_PARKING_DATA_ENTID'];
    ...
    const client = new __1.ParkhausBaselSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ParkingDataDirect.test.js.map