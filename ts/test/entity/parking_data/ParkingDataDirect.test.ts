...
describe('ParkingDataDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ParkhausBaselSDK({
  ...
  test('direct-load-parking_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-parking_data', setup.live)) return
  ...
  test('direct-list-parking_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-parking_data', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PARKHAUS_BASEL_TEST_PARKING_DATA_ENTID']
