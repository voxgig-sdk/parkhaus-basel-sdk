...
//     (\`new ParkhausBaselSDK(...)\` / \`ParkhausBaselSDK.test(...)\` become
//     \`ParkhausBaselSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ParkhausBaselSDK'
...
const TEST_SEED = {"entity":{"parking_data":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ParkhausBaselSDK(...)\` and any
// \`ParkhausBaselSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ParkhausBaselSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ParkhausBaselSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
