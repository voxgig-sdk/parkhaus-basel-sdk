// Command ParkhausBasel-cli is a CLI + REPL for the ParkhausBasel SDK,
...
const prompt = "parkhaus-basel"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "parking_data"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PARKHAUS_BASEL_APIKEY (api key), PARKHAUS_BASEL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PARKHAUS_BASEL_APIKEY carries the API key and
	// PARKHAUS_BASEL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PARKHAUS_BASEL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PARKHAUS_BASEL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewParkhausBaselSDK(opts)
