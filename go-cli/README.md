...
boru-driven command-line client **and** interactive REPL for the ParkhausBasel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parkhaus-basel-cli)
make build

# 2. See usage (words, entities, env vars)
./parkhaus-basel-cli --help

# 3. Provide credentials once, via the environment
export PARKHAUS_BASEL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./parkhaus-basel-cli list parking_data
./parkhaus-basel-cli load 1 parking_data            # {id:1} shorthand
./parkhaus-basel-cli load '{id:1}' parking_data       # explicit match map

# 5. Override the API base URL for a single call
PARKHAUS_BASEL_BASE=https://api.example.com ./parkhaus-basel-cli list parking_data

# 6. No arguments -> interactive REPL
./parkhaus-basel-cli
parkhaus-basel> list parking_data
parkhaus-basel> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parkhaus-basel-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/parkhaus-basel-cli list parking_data
   ```
...
```sh
./parkhaus-basel-cli load 1 parking_data          # scalar shorthand for {id:1}
./parkhaus-basel-cli load '{id:1}' parking_data     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `parkhaus-basel>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `parking_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/parkhaus-basel-cli`. |
...
(`./parkhaus-basel-cli <expr>`) and interactively (the REPL), and expressions compose the
