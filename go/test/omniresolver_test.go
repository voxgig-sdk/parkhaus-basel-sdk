...
import (
	...
	omni "github.com/voxgig-sdk/parkhaus-basel-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ParkhausBaselError); is {
				out := map[string]any{
					"name":    "ParkhausBaselError",
...
func omniCtx(arg any, client *sdk.ParkhausBaselSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ParkhausBaselSDK); is {
			client = live
