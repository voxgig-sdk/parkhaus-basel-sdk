...
local ParkingDataEntity = {}
ParkingDataEntity.__index = ParkingDataEntity


function ParkingDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ParkingDataEntity)
  self._name = "parking_data"
  self._client = client
...
function ParkingDataEntity:get_name()
  return self._name
end


function ParkingDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ParkingDataEntity.new(self._client, opts)
end
...
function ParkingDataEntity:mark_deleted()
  self._deleted = true
end


function ParkingDataEntity:deleted()
  return true == self._deleted
end


function ParkingDataEntity:data_set(args)
  if args ~= nil then
...
function ParkingDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ParkingDataEntity:match_set(args)
  if args ~= nil then
...
function ParkingDataEntity:match_get()
...
function ParkingDataEntity:stream(action, args, callopts)
...
---@param reqmatch ParkingDataLoadMatch
---@param ctrl? table
---@return ParkingData
---@return string? err
function ParkingDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ParkingDataListMatch
---@param ctrl? table
---@return ParkingData[]
---@return string? err
function ParkingDataEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ParkingDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
