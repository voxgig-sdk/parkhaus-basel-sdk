...
describe("ParkingDataDirect", function()
  it("should direct-list-parking_data", function()
    local setup = parking_data_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-parking_data", setup.live and "live" or "unit")
  ...
  it("should direct-load-parking_data", function()
    local setup = parking_data_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-parking_data", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function parking_data_direct_setup(mockres)
