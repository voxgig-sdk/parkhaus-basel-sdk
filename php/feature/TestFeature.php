...
// ParkhausBasel SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ParkhausBaselTestFeature extends ParkhausBaselBaseFeature
{
    ...
    public function init(ParkhausBaselContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ParkhausBaselContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ParkhausBaselParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ParkhausBaselContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ParkhausBaselContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ParkhausBaselParam::call($ctx, $k);
