...
class ParkingDataEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ParkhausBaselSDK::test(null, null);
        $ent = $testsdk->ParkingData(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = ParkhausBaselSDK::test($seed, null);
        $seen = iterator_to_array($base->ParkingData(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = ParkhausBaselConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = ParkhausBaselSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->ParkingData(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = parking_data_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "parking_data." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PARKHAUS_BASEL_TEST_PARKING_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $parking_data_ref01_data = null;
        if (count($parking_data_ref01_data_raw) > 0) {
...
function parking_data_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/parking_data/ParkingDataTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["parking_data01", "parking_data02", "parking_data03"] as $k) {
    ...
    $entid_env_raw = getenv("PARKHAUS_BASEL_TEST_PARKING_DATA_ENTID");
    ...
    if ($env["PARKHAUS_BASEL_TEST_LIVE"] === "TRUE") {
        ...
        $client = new ParkhausBaselSDK(Helpers::to_map($merged_opts));
    }
