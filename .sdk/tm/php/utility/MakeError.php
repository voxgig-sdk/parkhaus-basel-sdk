...
class ParkhausBaselMakeError
{
    public static function call(?ParkhausBaselContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ParkhausBaselContext([], null);
        }
        $op = $ctx->op ?? new ParkhausBaselOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ParkhausBaselResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ParkhausBaselError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ParkhausBaselError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ParkhausBaselError) {
