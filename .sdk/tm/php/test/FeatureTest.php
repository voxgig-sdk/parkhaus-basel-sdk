...
// ParkhausBasel SDK feature test
//
...
class FtCtrl extends ParkhausBaselControl
{
...
class FtClient
{
    ...
    private ParkhausBaselUtility $utility;

    public function __construct(ParkhausBaselUtility $utility, array $options)
    {
    ...
    public function get_utility(): ParkhausBaselUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ParkhausBaselContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ParkhausBaselContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ParkhausBaselContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ParkhausBaselConfig::make_feature($name);
    ...
    private static function buildUrl(ParkhausBaselSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ParkhausBaselError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ParkhausBaselResponse($fetched);
        }
    ...
    private function fail(ParkhausBaselContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ParkhausBaselContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
