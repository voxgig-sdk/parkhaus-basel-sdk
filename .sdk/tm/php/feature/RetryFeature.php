...
// ParkhausBasel SDK retry feature
...
class ParkhausBaselRetryFeature extends ParkhausBaselBaseFeature
{
    ...
    public function init(ParkhausBaselContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ParkhausBaselContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ParkhausBaselContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ParkhausBaselContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
