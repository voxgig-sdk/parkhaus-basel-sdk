...
[MCP](https://modelcontextprotocol.io) server exposing the ParkhausBasel SDK as
two agent tools — `parkhaus-basel_list` and `parkhaus-basel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parkhaus-basel-mcp)
make build

# 2. Provide credentials via the environment
export PARKHAUS_BASEL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user parkhaus-basel \
  -- /absolute/path/to/parkhaus-basel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./parkhaus-basel-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// parkhaus-basel_list: first page of records
{ "entity": "parking_data" }
{ "entity": "parking_data", "query": { } }

// parkhaus-basel_load: one record by id
{ "entity": "parking_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parkhaus-basel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user parkhaus-basel \
     -- "$PWD"/dist/*/parkhaus-basel-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `parkhaus-basel_list` and `parkhaus-basel_load` tools now appear
   in new sessions. Ask the agent to *"list parking_data using parkhaus-basel"*
   and it calls `parkhaus-basel_list` with `{"entity":"parking_data"}`.
...
```sh
export PARKHAUS_BASEL_APIKEY=sk_live_xxx            # API key
export PARKHAUS_BASEL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `parkhaus-basel_list` tool
...
### Call the `parkhaus-basel_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `parkhaus-basel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `parkhaus-basel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PARKHAUS_BASEL_APIKEY` | API key sent with every request. |
| `PARKHAUS_BASEL_BASE` | Optional override of the API base URL. |
...
```sh
./parkhaus-basel-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"parkhaus-basel_load","arguments":{"entity":"parking_data","query":{"id":1}}}}'
```
